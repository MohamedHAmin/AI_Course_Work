...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        next_q_value = 0
        # If next_action is not None, then next_state is a non-terminal state in which case, we consider the Q-value of next_state to be the Q-value of (next_state, next_action)
        if next_action is not None:
            next_q_value = self.compute_q(env, next_state, next_action)
        # Update the Q-table using the SARSA update rule
        self.Q[state][action] += self.learning_rate * (self.discount_factor * next_q_value - q_value + reward)
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        # Return the maximum Q-value of the given state
        q_values = [self.compute_q(env, state, action) for action in self.actions]
        return max(q_values)
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        q_value = self.compute_q(env, state, action)
        next_q_value = 0
        # If done is False, then next_state is a non-terminal state in which case, we consider the Q-value of next_state to be the maximum Q-value of next_state
        if not done:
            next_q_value = self.compute_utility(env, next_state)
        # Update the Q-table using the Q-Learning update rule
        self.Q[state][action] = (1 - self.learning_rate) * q_value + self.learning_rate * (self.discount_factor * next_q_value + reward)
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        return sum([0] + [features[feature] * self.weights[action][feature] for feature in features])
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        # If done is False, then next_state is a non-terminal state in which case, we consider the Q-value of next_state to be the maximum Q-value of next_state
        if not done:
            next_q_value = self.__compute_utility_from_features(self.feature_extractor.extract_features(env, next_state))
        # Update the weights using the Q-Learning update rule
        features = self.feature_extractor.extract_features(env, state)
        for feature in features:
            self.weights[action][feature] += self.learning_rate * (self.discount_factor * next_q_value - q_value + reward) * features[feature]
